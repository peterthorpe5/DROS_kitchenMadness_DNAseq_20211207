...
###### PCA of the goodeids
pca <- read_table2("DEML.ALL.PCA.eigenvec", col_names = FALSE)
eigenval <- scan('DEML.ALL.PCA.eigenval')
...
ggsave(filename = ("Kitchen_flies_PCA_default_colours_extra_samples.pdf"), plot = last_plot(), scale = 1, width = 7, height = 7, dpi = 1200, limitsize = TRUE)
...
b <- ggplot(pca, aes(PC1, PC2, col = species, label=species, shape=species))
      +   scale_shape_manual(values=1:nlevels(pca$species)) + geom_point(size = 3, alpha=0.6,
                                                                           position=position_jitter(width=0.06,height=0.06))
...
b <- b + coord_equal() 
b <-b + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
ggsave(filename = ("Kitchen_flies_PCA_default_colours_extra_samples2.pdf"), plot = last_plot(), scale = 1, width = 7, height = 7, dpi = 1200, limitsize = TRUE)
...
##### PLOT AS SHAPE FOR MIKE:
# shape doesnt work
pve <- data.frame(PC = 1:16, pve = eigenval/sum(eigenval)*100)
a <- ggplot(pve, aes(PC, pve)) + geom_bar(stat = "identity")
a <- a + ylab("Percentage variance explained") + theme_light()

b <- ggplot(pca, aes(PC1, PC2, shape = species, label=species)) + geom_point(size = 2, alpha=0.6,
                                                                           position=position_jitter(width=0.06,height=0.06))

#b <- b + scale_colour_brewer(palette = "Set1")
b <- b + coord_equal() 
b <-b + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
ggsave(filename = ("Kitchen_flies_PCA_extra_samples_diff_colour.pdf"), plot = last_plot(), scale = 1, width = 7, height = 7, dpi = 1200, limitsize = TRUE)
...
b <- ggplot(pca, aes(PC1, PC2, col = species, label=species)) + geom_point(size = 3, alpha=0.6,
                                                                           position=position_jitter(width=0.06,height=0.06)) + scale_color_brewer(palette="BrBG") 
...
b <-b + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
  ...
  ggsave(filename = ("Kitchen_flies_PCA_colour_blind.pdf"), plot = last_plot(), scale = 1, width = 7, height = 7, dpi = 1200, limitsize = TRUE)
